...
bool BallCreateInit(entt::registry& reg_) {
    ...
    for (size_t _slot = 0; _slot < 5000;_slot++) {
    ...
    LogInfo << reg_.view<BallTag, VectorChange, VectorComponent, PositionComponent, ObjectidComponent>().size() << FlushLog;
    return true;
}
bool BallCreateUpdate(const int64_t& dt_, entt::registry& reg_) {
    ...
    std::uniform_real_distribution<float> _random_speed_x_range(-10.f, 10.f);
    std::uniform_real_distribution<float> _random_speed_y_range(-10.f, 10.f);
    for (size_t _slot = 0; _slot < 0;_slot++) {
        auto _entity = reg_.create();
        reg_.emplace<BallTag>(_entity);
        reg_.emplace<VectorChange>(_entity);
        reg_.emplace<ObjectidComponent>(_entity, static_cast<uint64_t>(APIGetUniqueNumber()));
